    ...
    window.setGallerySettingsValues = function (data) {
        ...
        if (data.settings.page_elementable_type === 'App\\GallerySettings') {
            ...
            $.each(data.settings.page_elementable.image_items, function (e, i) {
                ...
                let input = '<input type="text" disabled data-id="'+ i.image.id +'" class="js-image-hover js-project-edit-gallery-image-filename-'+ (e + 1) +'" value="'+ i.image.filename +'"><button data-id="'+ i.image.id +'" class="img-edit-delete">Delete</button></input>';
